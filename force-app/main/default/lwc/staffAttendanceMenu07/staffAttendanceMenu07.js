...
import CHIKOKU_FIELD from '@salesforce/schema/KindergartenDiary__c.Lateness__c';             //園児日誌obj:遅刻
import SOTAI_FIELD from '@salesforce/schema/KindergartenDiary__c.LeaveEarly__c';             //園児日誌obj:早退
import IMPORTANTNOTESID_FIELD from '@salesforce/schema/KindergartenDiary__c.ImportantNotes__c';//園児日誌obj:要録情報ID
    ...
    ********************************/
    ChangeClassBus(event){
        ...
        if(this.Group === "Class"){
            ...
            console.log("■KariImportantNotesList4 Class");
            console.log(this.ImportantNotesList[0].Number__c);
            //KariImportantNotesList = this.ImportantNotesList.sort();
            KariImportantNotesList = this.sortsss(this.ImportantNotesList, 'Number__c', 'ASC');
        ...
        if(this.Group === "Bus"){
            ...
            console.log("■KariImportantNotesList4 Buss");
//            console.log(this.ImportantNotesList[0].Contact__r.Belongs__c);
            console.log(this.ImportantNotesList[0].Belongs__c);
            //KariImportantNotesList = this.ImportantNotesList.sort();
            KariImportantNotesList = this.sortsss(this.ImportantNotesList, 'BusDisplayOrder__c', 'ASC');
//            console.log(KariImportantNotesList[0].Contact__r.Belongs__c);
            console.log(KariImportantNotesList[0].Belongs__c);
            console.log(KariImportantNotesList);
    ...
    ********************************/
    ShowKindergartenDiary(result){
        ...
        for(let v of this.EnjiList) {
            ...
            SetEnjiList[i].Belongs__c = this.ImportantNotesList2[v].Belongs__c.substr(0,4);
//            SetEnjiList[i].Contact__r.PassageRoute__c = this.ImportantNotesList2[v].Contact__r.PassageRoute__c;
            SetEnjiList[i].PassageRoute__c = this.ImportantNotesList2[v].PassageRoute__c;
            ...
            if(this.ChildcareFeeList[v]){
                ...
                if(this.ChildcareFeeList[v].AttendanceStopDays__c ===undefined){this.ChildcareFeeList[v].AttendanceStopDays__c =0}
                SetEnjiList[i].AttendanceStopDays = this.ChildcareFeeList[v].AttendanceStopDays__c;
            }else{
            ...
            if (KindergartenDiaryB[v]) {
                ...
                SetEnjiList[i].Lateness = KindergartenDiaryB[v].Lateness__c;
                SetEnjiList[i].LeaveEarly = KindergartenDiaryB[v].LeaveEarly__c;
                SetEnjiList[i].ImportantNotesId = KindergartenDiaryB[v].ImportantNotes__c;
                /* END:該当日報の項目をセット */
            ...
            */
            ...
            /* 遅刻ボタン処理 */
            if(SetEnjiList[i].Lateness){
                SetEnjiList[i].LatenessClass='btn-square yellow';
            }else{
                SetEnjiList[i].Lateness=false;
                SetEnjiList[i].LatenessClass='btn-square gray';
            }

            /* 早退ボタン処理 */
            if(SetEnjiList[i].LeaveEarly){
                SetEnjiList[i].LeaveEarlyClass='btn-square yellow';
            }else{
                SetEnjiList[i].LeaveEarly=false;
                SetEnjiList[i].LeaveEarlyClass='btn-square gray';
            }
    ...
    ********************************/
    changeType(event){
        ...
        let Lateness;let LeaveEarly;
        let LatenessClass;let LeaveEarlyClass;
        let ImportantNotesId;
        ...
        Lateness = this.ShowEnjiList[indexs].Lateness;
        LatenessClass = this.ShowEnjiList[indexs].LatenessClass;
        LeaveEarly = this.ShowEnjiList[indexs].LeaveEarly;
        LeaveEarlyClass = this.ShowEnjiList[indexs].LeaveEarlyClass;
        ImportantNotesId = this.ShowEnjiList[indexs].ImportantNotesId;
        ...
        fields[STAFFID_FIELD.fieldApiName] = this.StaffId;  // 職員名簿ID
        ...
        /* クラス：遅刻の場合 */
        if(event.target.dataset.group === 'Class2' && event.target.dataset.value === '遅刻'){
            /* ON・OFFスイッチ */
            if(this.ShowEnjiList[indexs].Lateness){
                fields[CHIKOKU_FIELD.fieldApiName] = false;                                       //遅刻チェック
                Lateness=false;
                LatenessClass='btn-square gray';
            }else{
                fields[CHIKOKU_FIELD.fieldApiName] = true;                                         //遅刻チェック
                Lateness=true;
                LatenessClass='btn-square yellow';
            }
         }
        /* END:クラス：遅刻の場合 */

        /* クラス：早退の場合 */
        if(event.target.dataset.group === 'Class2' && event.target.dataset.value === '早退'){
            /* ON・OFFスイッチ */
            if(this.ShowEnjiList[indexs].LeaveEarly){
                fields[SOTAI_FIELD.fieldApiName] = false;                                       //早退チェック
                LeaveEarly=false;
                LeaveEarlyClass='btn-square gray';
            }else{
                fields[SOTAI_FIELD.fieldApiName] = true;                                         //早退チェック
                LeaveEarly=true;
                LeaveEarlyClass='btn-square yellow';
            }
         }
        /* END:クラス：早退の場合 */
        ...
        if(event.target.dataset.group === 'Bus' && event.target.dataset.value === '登園'){
            ...
            if(this.ShowEnjiList[indexs].AttendingSchool){
                ...
                fields[AS_FIELD.fieldApiName] = '出席';                                          //出席扱いとする
                AttendanceSchedule = '出席';
                fields[ASTIME_FIELD.fieldApiName] = timesstump;                                 //出席にした時間
        ...
        if(event.target.dataset.group === 'Bus' && event.target.dataset.value === '降園'){
             ...
             if(this.ShowEnjiList[indexs].GoingBack){
                fields[KOUEN_FIELD.fieldApiName] = false;                                        //降園チェック
                fields[KOUENTIME_FIELD.fieldApiName] = '';                                       //降園時間
                GoingBack=false;
        ...
        if(event.target.dataset.group === 'Bus' && event.target.dataset.value === '行き利用しない'){
            ...
            if(this.ShowEnjiList[indexs].NoAttendingSchool){
               fields[GONOTUSE_FIELD.fieldApiName] = false;                                        //行き利用なしチェック
               NoAttendingSchool=false;
               NoAttendingSchoolClass='btn-square gray';
           }else{
               fields[GONOTUSE_FIELD.fieldApiName] = true;                                         //行き利用なしチェック
        ...
        if(event.target.dataset.group === 'Bus' && event.target.dataset.value === '帰り利用しない'){
            ...
            if(this.ShowEnjiList[indexs].NoGoingBack){    
                fields[BACKNOTUSE_FIELD.fieldApiName] = false;                                        //帰り利用なしチェック
                NoGoingBack=false;
                NoGoingBackClass='btn-square gray';
            }else{
                fields[BACKNOTUSE_FIELD.fieldApiName] = true;                                         //帰り利用なしチェック
                NoGoingBack=true;
        ...
        if(fields[ID_FIELD.fieldApiName] === undefined || fields[ID_FIELD.fieldApiName]=== '' || fields[ID_FIELD.fieldApiName]=== 'undefined'){
            ...
            .then(result => {   //レコード作成に成功したら
                ...
                this.ShowEnjiList[indexs].Lateness = Lateness;
                this.ShowEnjiList[indexs].LatenessClass = LatenessClass;
                this.ShowEnjiList[indexs].LeaveEarly = LeaveEarly;
                this.ShowEnjiList[indexs].LeaveEarlyClass = LeaveEarlyClass;
    ...
    ********************************/
    ...
    /********************************
        要録情報レコードページへ
    ********************************/
    JampToImportantNotes(event){
        if(event.target.dataset.importantnotesid === undefined || event.target.dataset.importantnotesid === "" || event.target.dataset.importantnotesid === "undefined"){
            return;
        }
        window.open( "/" + event.target.dataset.importantnotesid ) ;
    }
    /* END:要録情報レコードページへ */
